...
export const Years = () => {
  ...
  const yearsDifference = Math.round(currentYear - startYear);
...
從2005年第一份資訊工程師的工作開始，至今已近<Years />個年頭的時間，在資訊界裡載浮載沉。從程式設計師到企業總部高階主管的軟體幕僚，目前尚無法定義自己的工作與角色到底是什麼，但仍對於各種資訊系統開發技術仍充滿熱情。
